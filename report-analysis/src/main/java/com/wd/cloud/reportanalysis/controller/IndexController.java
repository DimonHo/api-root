...
//    }
    ...
    @ApiOperation(value = "发文量、分区、被引频次对比分析（esi）")
    @ApiImplicitParams({
    	@ApiImplicitParam(name = "block", value = "是否本校(本校:ourschool;多校对比:contrast)", dataType = "String", paramType = "query"),
    	@ApiImplicitParam(name = "plate", value = "板块  paper:论文对比分析 ;esi:ESI对比分析", dataType = "String", paramType = "query"),
            @ApiImplicitParam(name = "act", value = "分析类型;amount：发文量;partition：分区;cited：被引频次", dataType = "String", paramType = "query"),
    ...
    public ResponseModel compare(HttpServletRequest request) {
    	...
    	ResourceLabel resource = new ResourceLabel(request);
    	String type = resource.getType();
        String esiType = resource.getEsiType();
        
    	if(resource.getAct().equals("issue")){
    		List<Map<String, Object>> list = analysisByDBService.getIssue(0, 1);
            result.put("content", list);
    	} else
    	
        
        if(esiType.equals("analysis")) {
        	Map<String, Object> scidMap = new HashMap<>();
        	String scid = resource.getScid();
        	result = analysisByDBService.analysisEsiPaper(Integer.parseInt(scid), resource.getCategory(), resource.getAct(),resource.getIssue(), 0);
//        	result.put("content", scidMap.get("content"));
        } else {
        	Map<String, Object> scidMap = new HashMap<>();
	        resource.getScids().forEach(scid -> {
	            School school = schoolService.findByScid(Integer.parseInt(scid));
    ...
    public ResponseModel compareEsiPaper(HttpServletRequest request) {
        ResourceLabel resource = new ResourceLabel(request);
        String type = resource.getType();
        String esiType = resource.getEsiType();
        Map<String, Object> result = new HashMap<String, Object>();
        if(esiType.equals("analysis")) {
        	Map<String, Object> scidMap = new HashMap<>();
        	String scid = resource.getScid();
        	scidMap = analysisByDBService.analysisEsiPaper(Integer.parseInt(scid), resource.getCategory(), resource.getAct(),resource.getIssue(), 0);
        	result.put("content", scidMap.get("content"));
        } else {
        	Map<String, Object> scidMap = new HashMap<>();
	        resource.getScids().forEach(scid -> {
