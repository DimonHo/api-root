...
@Component("subjectQuery")
public class SubjectQueryBuildStrategy implements QueryBuilderStrategyI {
	...
	public QueryBuilder execute(QueryCondition queryCondition) {
		...
		subBoolQueryBuilder.should(QueryBuilders.nestedQuery("documents",
			...
			.defaultField("documents.subjects").minimumShouldMatch("80%").defaultOperator(Operator.AND)
			, ScoreMode.Max));
